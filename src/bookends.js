...
function subText(C) {
  const { tf, minDim, up, subline, artist, year } = C;
  return tf.get(up(subline || (artist ? `PORTFOLIO ${year}` : `SELECTED WORKS ${year}`)), {
    family: 'mono', size: Math.round(minDim * 0.024), weight: 600, tracking: 0.3,
  });
}
...
export const OPENER_LABELS = { montage: 'モンタージュ', type: 'タイプ', shutter: 'シャッター', countdown: 'カウント', knockout: '抜き文字' };
export const CLOSER_LABELS = { grid: 'グリッド', filmstrip: 'フィルム', stack: 'スタック', knockout: '抜き文字' };
